...
###########################
	...
	then
		...
		/root/pg_backup/azcopy sync "$FINAL_BACKUP_DIR"$DATABASE/current/$DATABASE"$FINAL_BACKUP_NAME.dump" "$BLOB_URL/$CURRENT_DATE/$DATABASE/$BLOB_SAS"
		
		echo -e " "
		echo -e "Sync current backup to azure blob"
		/root/pg_backup/azcopy sync "$FINAL_BACKUP_DIR"$DATABASE/current/$DATABASE"$FINAL_BACKUP_NAME.dump" "$BLOB_URL/$BLOB_SAS"
